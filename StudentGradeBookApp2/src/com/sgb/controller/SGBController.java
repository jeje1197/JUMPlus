...
public class SGBController {
	...
	public static StudentDaoSql studentDao = new StudentDaoSql();
	...
	public static void loginAsStudent() {
		ColorPrinter.println(ColorPrinter.CYAN, PrettyFormatter.format(new String[] {"STUDENT LOGIN"}));

		int studentId = -1;
		try {

			ColorPrinter.print(ColorPrinter.GREEN, "Enter your student id: ");
			studentId = ConsoleScanner.readInt(0, 1000000);
		} catch (InvalidInputException e) {
			ColorPrinter.println(ColorPrinter.RED, e.getMessage() + "\n");
			return;
		}

		StudentUser student = studentDao.getStudentById(studentId);
		if (student != null) {
			ColorPrinter.println(ColorPrinter.YELLOW, "Successfully logged in!\n");
			StudentController.displayInformation(studentDao, student);
		} else {
			ColorPrinter.println(ColorPrinter.RED, "Failed to log in!\n");
		}
	}
